...
class BM25Search:
    ...
    def search(self, query: str, top_k: int = BM25_TOP_K) -> list[SearchResult]:
        ...
        return [SearchResult(text=self.documents[i]["text"], score=float(scores[i]), metadata=self.documents[i]["metadata"], method="bm25") for i in top_indices]
...
class DenseSearch:
    ...
    def index(self, chunks: list[dict], collection: str = COLLECTION_NAME) -> None:
        ...
        points = [PointStruct(id=i, vector=v.tolist(), payload={**c["metadata"], "text": c["text"]}) for i, (c, v) in enumerate(zip(chunks, vectors))]
    ...
    def search(self, query: str, top_k: int = DENSE_TOP_K, collection: str = COLLECTION_NAME) -> list[SearchResult]:
        ...
        try:
            ...
            return [SearchResult(text=hit.payload.get("text", ""), score=float(hit.score), metadata=hit.payload, method="dense") for hit in hits]
        except Exception:
            return []
